<template>
  <div
    ...
    <div class="table-box">
      <!-- <mulite-entry @submitInput="submitBarcode" /> -->
      <base-table
...
@Component({
  name: 'goods-detail',
  components: {
    fileImport: () => import('../components/file-input.vue'),
...
export default class App extends mixins(vuexMixins) {
  ...
  tableData: any[] = []
  goodsSkuData: any[] = []
  goodsSkuTable: TableHeaderConfig[] = []
  referenceTable: TableHeaderConfig[] = []
  referenceData: any[] = []
  goodsInputIndex: number = 0
  ...
  watchGoodsDetailData(val, oldval) {
    console.log(val)
    this.submitBarcode(val)
  }
  ...
  @Watch('formData.billRealGoodsList', { immediate: true })
      ...
      this.tableHeader = res.data
        .map((item) => {
          ...
          const obj: any = {
            ...
            value: item.fieldName === 'priceTypeName' ? 'priceTypeId' : item.fieldName,
          ...
          if (item.fieldName === 'priceTypeName') {
            const priceOptions:any = []
            const priceTypes = (this.dropConfig as any).priceTypeIds
            for (const key in priceTypes) {
              priceOptions.push({
                label: priceTypes[key].name,
                value: priceTypes[key].id
              })
            }
            obj.options = priceOptions
          }
          // 如果有自定义字段
          if (customer) {
  ...
  async submitBarcode({ barcode, number, currentType, allCode }) {
    switch (+currentType) {
      case 1: // 自动识别
        this.getAuto(barcode, number, allCode)
        break
      case 2: // SKU
        this.getSku(barcode, number, allCode)
        break
      case 3: // 条形码
        this.getBarCode(barcode, number, allCode)
        break
      case 4:// 唯一码
        this.getUniCode(barcode, number, allCode)
        break
      case 5: // 箱号
        this.getBoxCode(barcode, number, allCode)
    }
  }
  async getAuto(barcode, number, allCode) {
    const res = await this._post({
      url: `${apiUrl.scanAuto}`,
      data: {
        data: {
  ...
  async getBarCode(barcode, number, allCode) {
    const res = await this._post({
      url: `${apiUrl.scan}`,
      data: {
        data: {
    ...
    if (res.code === 0) {
      ...
      // 往列表里面添加一条数据
        ...
        this.tableData.push(Object.assign({}, otherData, { goodsFields }, { customFields }))
        matchObjIndex = this.tableData.length - 1
      }
      // 根据该条形码 获取到的 颜色 内长 找到对应的 SKU 并且添加数量
      this.getOnlySkuWithValue(matchObjIndex, barcodeScanResp, number, goodsSkuDto, allCode)
      this.tableKey = this.tableKey + 1
      // 计算总数量 如果有填写扫码数量
      this.editNum(matchObjIndex, number, goodsSkuDto, allCode)
  ...
  async getUniCode(barcode, number, allCode) {
    const res = await this._post({
      url: `${apiUrl.uniCode}`,
      data: {
        data: {
  ...
  async getSku(barcode, number, allCode) {
    const res = await this._post({
      url: `${apiUrl.sku}`,
      data: {
        data: {
  ...
  async getBoxCode(barcode, number, allCode) {
    if (!this.formData.terminalLogistics.toChannelId) {
      this._message({ message: '收货渠道不能为空', type: 'warning' })
      return
    }
    const res = await this._post({
      url: `${apiUrl.boxScan}`,
      data: {
        data: {
          keyWord: barcode,
          moduleId: globalConfigs.moduleId,
          toChannelId: this.formData.terminalLogistics.toChannelId
        }
      }
    })
    console.log('箱号', res)
  ...
  singleGoodsInput(value, row) {
    // this.$set(row, 'totalPrice', row.quantity * Number(row.balancePrice))
    row.totalPrice = value * Number(row.balancePrice)
  }
  // 根据该条形码解析到的 颜色 内长 找到 billSizeList sku矩阵信息 对应的skn 并且添加数量
  getOnlySkuWithValue(matchObjIndex, barcodeScanResp, number, goodsSkuDto, allCode) {
    ...
    if (currentTable.billSizeList) {
      if (!allCode) {
        ...
        if (result) {
          if (result[barcodeScanResp.fieldName]) {
            // 如果是相同尺码则累加
            result[barcodeScanResp.fieldName] = result[barcodeScanResp.fieldName] + number
          } else {
            // 不同尺码则添加尺码
            result[barcodeScanResp.fieldName] = number
          }
        } else {
        ...
        const sizeList = goodsSkuDto.headerList.slice(2, goodsSkuDto.headerList.length);
        (sizeList || []).forEach(em => {
          const { value } = em
          const result = currentTable.billSizeList.find(em => {
            return em.colorId === barcodeScanResp.colorId && em.longId === barcodeScanResp.longId
      ...
      if (!allCode) {
        ...
        (sizeList || []).forEach(em => {
          const { value } = em
          sizeOBJECT[value] = number > 0 ? number : 0
        })
        billSizeList.push({
          colorId: barcodeScanResp.colorId,
          colorName: `${barcodeScanResp.colorCode}-${barcodeScanResp.colorName}`,
  ...
  // 计算数量和金额
  editNum(index, num, goodsSkuDto, allCode) {
    const row = this.tableData[index]
    const sizeList = goodsSkuDto.headerList.slice(2, goodsSkuDto.headerList.length)
    const newQuantity = row.quantity ? Number(row.quantity) + (num * (allCode ? sizeList.length : 1)) : (num * (allCode ? sizeList.length : 1))
    this.$set(row, 'quantity', newQuantity >= 0 ? newQuantity : 0)
    this.$set(row, 'totalPrice', (row.quantity * Number(row.balancePrice)).toFixed(2))
  }
  // 扫码记录新增
  ...
  async viewDetail(row) {
    console.log(row, this.skuHeaderObj)
    // 如果为普通物料则执行 显示已输入的数量
    ...
    if (this.skuHeaderObj['' + row.goodsId]) {
      ...
      if (res.code === 0) {
        this.currentSizeTable = Object.assign({}, this.getSizeTableHeader(res.data))
        this.$set(this.skuHeaderObj, '' + row.goodsId, this.currentSizeTable)
        // this.skuHeaderObj[row.goodsId] = this.currentSizeTable
      }
    }
    this.currentSizeData = row.billSizeList
    ;(this.$refs.sizeTable as any).doLayout()
  }
  // 获取sku矩阵的尺码头
  getSizeTableHeader(arr) {
    ...
    console.log(this.currentSizeTable)
  ...
  async getDropConfig() {
    const res:any = await this._get({ url: apiUrl.getSelect, params: {
      moduleId: globalConfigs.realModuleId
    }})
    if (res.code === 0) {
      const data = res.data
      this.dropConfig = data
    }
  }
}
</script>
