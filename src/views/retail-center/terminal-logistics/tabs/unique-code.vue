...
export default class App extends mixins(vuexMixins) {
 ...
 tableHeader: TableHeaderConfig[] = detailConfig.uniqueCode
 ...
 onv_statusChange(val, oldval) {
  ...
  // mounted() {
  //   console.log('mounted adjustment-detail')
  ...
  async getAdjustmentDetail() {
    ...
    const res = await this._post({ url: apiUrl.getAdjustmentDetail, params: {
      salePlanBillId: this.v_id.split('_')[0],
