...
export default class App extends mixins(vuexMixins) {
  ...
  submitBarcode({ barcode, number }) {
    // 需要先选择渠道编号/价格类型
    if (!this.formData.salePlain.priceTypeId) {
      this._message({ message: '请先选择价格类型', type: 'warning' })
      return
    }
    if (!this.formData.salePlain.channelId) {
  ...
  async addGoods(barcode, number) {
    const res = await this._post({
      ...
      data: {
        data: {
          keyword: barcode,
          channelId: this.formData.salePlain.channelId,
          priceTypeId: this.formData.salePlain.priceTypeId,
          moduleId: globalConfigs.realModuleId
        }
      }
    })
