...
export default class App extends mixins(vuexMixins) {
  ...
  async addGoods(barcode, number) {
    ...
    if (this.salePlanBillId) {
      const salePlanArray = JSON.parse(JSON.stringify(this.salePlanArray))
      if (salePlanArray.length === 0) {
      ...
      res = await this._post({
        ...
        data: {
          data: {
            keyword: barcode,
            channelId: this.formData.noticeBill.channelId,
            priceTypeId: this.formData.noticeBill.priceTypeId,
            moduleId: this.m_moduleId
          }
        }
      })
      if (!~salePlanArray.findIndex(item => { return item.goodsId === res.data.goodsId })) {
        this.$message.warning('该条形码不在销售计划单的货品范围内，不允许添加！')
        return
      }
    } else {
      res = await this._post({
        ...
        data: {
          data: {
            keyword: barcode,
            channelId: this.formData.noticeBill.toChannelId,
            priceTypeId: this.formData.noticeBill.priceTypeId,
            moduleId: this.m_moduleId
          }
        }
  ...
  async viewDetail(row) {
    ...
    row.billSizeList.forEach(item => {
      item.colorName_ = item.colorName_ || item.colorName
      item.colorName = `${item.colorCode}-${item.colorName_}`
    })
