...
export default class App extends Vue {
  ...
  async fuzzySearchFn() {
    let res :any
    console.log('销售计划单', this.salePlanBillId)
    if (this.salePlanBillId) {
      let salePlanArray = JSON.parse(JSON.stringify(this.salePlanArray))
      // 销售计划单据无货品
      ...
      // 搜索条件匹配为空[过滤]
      if (this.goodsCode) {
        const result = salePlanArray.filter(item => {
          return item.goodsCode.indexOf(this.goodsCode) > -1
        })
        if (result.length === 0) {
          this.$message.warning('搜索条件在当前销售计划单下无匹配货品，请重新输入搜索条件！')
          return
        } else {
          salePlanArray = result
        }
      }
      res = {
        code: 0,
        data: salePlanArray,
        total: salePlanArray.length,
        pageSize: salePlanArray.length,
        pageCount: 1
      }
      res.data = res.data.map(row => {
        return {
          ...
          goodsFields: {
            goodsBuildDate: '2020-03-26',
            goodsStyleName: null }
        }
      })
    } else {
      res = await this._post({ url: apiUrl.goodsEntry, data: {
        data: {
          keyword: this.goodsCode,
          moduleId: this.$route.meta.moduleId,
          channelId: this.formData.toChannelId,
