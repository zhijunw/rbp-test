...
import { detailConfig, globalConfigs, recordLogObj } from './configs'
...
export default class detail extends mixins(operationMixins, vuexMixins) {
    ...
    edit() {
      if (this.type === 'sendPlan') {
        this.$message.warning('发货指令单不允许修改')
        return
      }
      this.m_edit()
    }
    ...
    async m_delete(config: ListBtnApiConfig) {
      const res = await this._post({
        url: config.apiUrl!, data: {
          data: {
            ids: [this.billId],
            moduleId: this.globalConfigs.realModuleId
          }
        }
      })
    ...
    async m_record() {
      (this.$refs.recordLog as any).open({
        args: { billNo: this.formData.billNo, billId: this.billId, moduleId: this.globalConfigs.realModuleId },
        from: 'detail'
      })
    }
}
