...
import commonApi from '@/api/common-api'
  ...
  @Component({
    ...
    components: {
      ...
      goodsPosition: () => import('../components/goods-position.vue')
    }
  })
export default class App extends mixins(vuexMixins) {
    @Prop() noticeBillData: any[]
    ...
    // 货品定位
    goodsPosition(string: string) {
      const target = this.tableData.find(item => {
        return item.goodsCode.indexOf(string) > -1 || item.goodsName.indexOf(string) > -1
      })
      target && (() => {
        console.log('匹配');
        (this.$refs['goodsDetailTable'] as any).setHighLightRow(target)
        this.viewDetail(target)
      })()
    }
    ...
    // 【价格类型或者渠道】变更以致于更新货品表的售价跟折扣goodsIdList
    async updateGoodsByChannelIdOrPriceType(channelId, priceTypeId) {
      if (channelId && priceTypeId) {
        ...
        const res = await this._post({ url: apiUrl.goodsEntry, data: prop })
        if (res.code === 0) {
          for (let n = 0; n < this.tableData.length; n++) {
            for (let m = 0; m < res.data.length; m++) {
              if (this.tableData[n].goodsId === res.data[m].goodsId) {
                Object.assign(this.tableData[n], res.data[m])
                break
              }
            }
          }
    ...
    submitBarcode({ barcode, number }) {
      // // 需要先选择渠道编号/价格类型
      // if (!this.formData.deliver.priceTypeIds) {
      //   this._message({ message: '请先选择价格类型', type: 'warning' })
      //   return
      // }
      // if (!this.formData.deliver.toChannelId) {
      //   this._message({ message: '请先选择到货渠道', type: 'warning' })
      //   return
      // }
    ...
    // 列表新增 条形码 然后滚动到最底行
    async addGoods(barcode, number, allLong) {
      const { barcodeScanResp, goodsSkuDto, goodsFields, ...otherData } = barcode
      ...
      this.getOnlySkuWithValue(matchObjIndex, barcodeScanResp, number, goodsSkuDto, allLong)

      this.tableKey = this.tableKey + 1

      // 计算总数量 如果有填写扫码数量
      this.editNum(matchObjIndex, number, goodsSkuDto, allLong)
      ...
      this.tableData[matchObjIndex].noticeId = this.formData.deliver.noticeId
      // 往扫码记录里面添加一条数据
    ...
    goodsInput(row, notShowDialog) {
      ...
      // 判断该货品是否在指令单列表范围内 noticeBillData
      if (this.noticeBillData.length > 0) {
        if (!(this.noticeBillData.findIndex((item) => item.goodsId === row.goodsId) > -1)) {
          this.$message.warning('录入的货品不在该指令单范围内，请重新选择录入')
          return
        }
      }
      // 根据条码匹配到数据
    ...
    getOnlySkuWithValue(matchObjIndex, barcodeScanResp, number, goodsSkuDto, allLong) {
    ...
    // 计算数量和金额
    editNum(index, num, goodsSkuDto, allLong) {
      const row = this.tableData[index]
      const sizeList = goodsSkuDto.headerList.slice(2, goodsSkuDto.headerList.length)
      const newQuantity = row.quantity ? Number(row.quantity) + (num * (allLong ? sizeList.length : 1)) : (num * (allLong ? sizeList.length : 1))
